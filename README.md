...
2026-09-17 기준 한국어 19페이지와 영어 19페이지의 사용자 설명서입니다. 한국어는 루트 경로, 영어는 `en/`의 같은 파일명에 둡니다. `docs.json`의 `navigation.languages`가 언어 선택과 탐색을 구성합니다.
...
38개 내비게이션 경로, frontmatter, 내부 링크와 MDX 컴포넌트 짝은 정적 검사에서 통과했습니다. Mintlify 빌드와 내부 링크 검사는 2026-09-17에 다시 확인했습니다.
...
## 2026-09-17 로컬 MCP 안내

- 한·영 `ask-with-ai` 페이지를 추가하고 백업·AI·데이터 안내에서 연결했습니다. 갈피 앱 자체 기능이 아닌 별도 로컬 실험판임을 명시합니다.
- 공개 저장소의 `tools/galpi-mcp`에 읽기 전용 도구 소스를 포함했습니다. 개인 백업 파일, 로컬 가상환경, 개인 경로는 포함하지 않습니다.
- Mintlify `validate`와 `broken-links`, 합성 백업 기반 MCP 왕복 테스트가 통과했습니다.
